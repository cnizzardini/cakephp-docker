...
init: do.copy
	...
	@docker-compose build --build-arg UID=$(shell id -u) --build-arg ENV=dev --build-arg HOST_OS=$(uname -s)
	@$(DC_UP)
init.nocache: do.copy
	@printf $(DOCKER_ICO) && printf "$(GOOD)running docker build --no-cache and up $(E)"
	@mkdir -p app && touch app/.gitkeep
	@docker-compose build --build-arg UID=$(shell id -u) --build-arg ENV=dev --no-cache --build-arg HOST_OS=$(uname -s)
	@$(DC_UP)
